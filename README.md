...
echo "6.4.1" > /opt/echoes/firmware/version.txt
```
...
```bash
./build.sh version patch     # 6.4.1 → 6.4.2 (updates FIRMWARE_VERSION in main/ota.h)
...
| `./build.sh version patch` | Increment patch version in `main/ota.h` (e.g. 6.4.1 → 6.4.2) |
| `./build.sh version minor` | Increment minor version (e.g. 6.4.1 → 6.5.0) |
