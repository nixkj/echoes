...
echo "7.2.2" > /opt/echoes/firmware/version.txt
```
...
```bash
./build.sh version patch     # 7.2.2 → 7.2.3 (updates FIRMWARE_VERSION in main/ota.h)
...
| `./build.sh version patch` | Increment patch version in `main/ota.h` (e.g. 7.2.2 → 7.2.3) |
| `./build.sh version minor` | Increment minor version (e.g. 7.2.2 → 7.3.0) |
| `./build.sh version major` | Increment major version (e.g. 7.2.2 → 8.0.0) |
...
I (580) MAIN: Firmware Version: 7.2.2
I (584) MAIN: ========================================
I (606) RCFG: Remote config initialised with defaults
I (606) MAIN: Initializing system...
I (609) ECHOES: BH1750 light sensor detected at 0x23
I (609) ECHOES: Hardware detected: FULL (BH1750 + Audio)
I (615) MARKOV: Loaded transition counts from NVS (578 bytes)
I (619) MARKOV: Markov chain ready — 17 states, current: STARTUP
I (625) MARKOV: Top transitions from STARTUP:
I (629) MARKOV:   #1  IDLE+DAWN             21.3%
...
I (7351) STARTUP: Reset reason: POWERON (1)
...
I (7460) STARTUP: Startup report sent successfully (attempt 1)
I (7466) MAIN: Startup report sent successfully
I (7670) MAIN: Task watchdog: 120s timeout, panic on trigger
I (7670) MAIN: WiFi connected successfully
I (7671) MAIN: Fetching remote configuration...
I (7727) RCFG: Config applied — vol=0.50 whistle=2000Hz voice=200Hz poll=500ms
I (7728) MAIN: Remote config applied successfully
I (7729) ECHOES: Audio detection task started
I (7733) ECHOES: 🎤 Listening for whistles, voice, claps, and birdsong...
```
...
**7.2.2** Current version
- Improved build.sh that automates updating the README.md
- Improve WDT logic further based on observing the ESP-NO logs over more than 1 hour
...
- Include additional diagnostic information to try get to the bottom of minimal nodes dropping
- Add a debug option to echoes-server.py
- Refine lux measurement broadcast

**7.0.8** 
- Attempt to further improve WDT/ISR and Wi-Fi reconnect on minimal nodes
- Align the second and fourth groups on the UI

**6.5.10**
- Stable Wi-Fi
- Disable Markov from minimal nodes (no point)
- Other refinements to try prevent minimal nodes from stalling
- Fix volume parameter bug, tidy up the UI
- Improve logic for Wi-Fi reconnect if dropped
...
- For minimal nodes: additional logic for Wi-Fi disappearing, including heartbeat
