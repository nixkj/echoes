...
```bash
./build.sh version patch     # 7.1.0 → 7.1.1 (updates FIRMWARE_VERSION in main/ota.h)
...
idf.py build
cp build/echoes.bin /opt/echoes/firmware/echoes.bin
echo "7.1.0" > /opt/echoes/firmware/version.txt
```
...
| `./build.sh version patch` | Increment patch version in `main/ota.h` (e.g. 7.1.0 → 7.1.1) |
| `./build.sh version minor` | Increment minor version (e.g. 7.1.0 → 7.2.0) |
| `./build.sh version major` | Increment major version (e.g. 7.1.0 → 8.0.0) |
...
I (580) MAIN: Firmware Version: 7.1.0
I (584) MAIN: ========================================
